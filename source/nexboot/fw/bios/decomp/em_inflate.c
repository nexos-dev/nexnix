 ...
 */
static inline void em_lsb_bitreader_init (em_lsb_bitreader_t* pBitReader,
                                          const unsigned char* pInBlock,
                                          const unsigned char* pInBlockEnd)
{
...
{
#ifdef EM_LSB_BITREADER_64BIT_SHIFTER
    if (pBitReader->nDecShifterBitCount <= 32 &&
        (pBitReader->pInBlock + 4) <= pBitReader->pInBlockEnd)
    {
#if defined(_M_X64) || defined(__x86_64__)
        /* Read unaligned word */
        pBitReader->nDecShifterData |=
            (((em_lsb_bitreader_shifter_t) (*((unsigned int*) pBitReader->pInBlock)))
             << pBitReader->nDecShifterBitCount);
        pBitReader->nDecShifterBitCount += 32;
        pBitReader->pInBlock += 4;
#else
        /* Read bytes */
        pBitReader->nDecShifterData |= (((em_lsb_bitreader_shifter_t) (*pBitReader->pInBlock++))
                                        << pBitReader->nDecShifterBitCount);
 ...
 */
static inline unsigned int em_lsb_bitreader_get_bits (em_lsb_bitreader_t* pBitReader,
                                                      const int nBits)
{
    if (pBitReader->nDecShifterBitCount < nBits)
    {
        if (pBitReader->pInBlock < pBitReader->pInBlockEnd)
        {
            ...
            {
                pBitReader->nDecShifterData |=
                    (((em_lsb_bitreader_shifter_t) (*pBitReader->pInBlock++))
                     << pBitReader->nDecShifterBitCount);
                pBitReader->nDecShifterBitCount += 8;
            }
        }
...
static int em_lsb_bitreader_byte_align (em_lsb_bitreader_t* pBitReader)
{
    ...
    while (pBitReader->nDecShifterBitCount >= 8)
    {
        ...
        pBitReader->nDecShifterBitCount -= 8;
        pBitReader->pInBlock--;
        if (pBitReader->pInBlock < pBitReader->pInBlockStart)
            return -1;
    }
...
/** Number of most significant huffman codeword bits that are used to skip the initial code length
 * finding steps */
 ...
 */
static int em_lsb_huffman_decoder_prepare_table (em_lsb_huffman_decoder_t* pDecoder,
                                                 unsigned int* pRevSymbolTable,
                                                 const int nReadSymbols,
                                                 const int nSymbols,
                                                 const unsigned char* pCodeLength)
{
    int nNumSymbolsPerLen[16];
    int i;

    if (nReadSymbols < 0 || nReadSymbols > MAX_SYMBOLS || nSymbols < 0 || nSymbols > MAX_SYMBOLS ||
        nReadSymbols > nSymbols)
        return -1;
 ...
 * @return 0 for success, -1 for failure
 */
static int em_lsb_huffman_decoder_finalize_table (em_lsb_huffman_decoder_t* pDecoder,
                                                  unsigned int* pRevSymbolTable)
{
    ...
    /* Re-create canonical huffman codewords and create left-justified words for
     * em_lsb_huffman_decoder_read_value() */
    ...
    while (i < pDecoder->nNumSorted)
    {
        ...
        while (i < pDecoder->nStartingPos[nCanonicalLength])
        {
            ...
            if (nCanonicalCodeWord >= (1U << nCanonicalLength))
                return -1;

            /* Write accelerated symbol value + codeword len for the (upside down) top
             * NFASTSYMBOLBITS bits of the codeword, at all bit positions */
            if (nCanonicalLength <= NFASTSYMBOLBITS)
            {
                ...
                nRevWord =
                    ((nCanonicalCodeWord & 0x5555) << 1) | ((nCanonicalCodeWord & 0xaaaa) >> 1);
                ...
                while (nSlots)
                {
                    pDecoder->nFastSymbol[nRevWord] =
                        (pRevSymbolTable[i] & 0xffffff) | (nCanonicalLength << 24);
 ...
 */
static inline unsigned int em_lsb_huffman_decoder_read_value (em_lsb_huffman_decoder_t* pDecoder,
                                                              const unsigned int* pRevSymbolTable,
                                                              em_lsb_bitreader_t* pBitReader)
{
    ...
    if (nFastSymAndBits)
    {
        /* Fast path: the symbol len is <= NFASTSYMBOLBITS, we have a symbol and bit length for the
         * (upside-down) MSB bits */
        em_lsb_bitreader_consume_bits (pBitReader, nFastSymAndBits >> 24);
        return nFastSymAndBits & 0xffffff;
    }

    /* The symbol len is larger than NFASTSYMBOLBITS, decode using the slower path. By definition,
     * the more frequent symbols are shorter and in the fast path. */
 ...
 * @return 0 for success, -1 for failure
 */
static int em_lsb_huffman_decoder_read_raw_lengths (const int nLenBits,
                                                    const int nReadSymbols,
                                                    const int nSymbols,
                                                    unsigned char* pCodeLength,
                                                    em_lsb_bitreader_t* pBitReader)
{
    static const unsigned char nCodeLenSymIndex[NCODELENSYMS] =
        {16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15}; /* Order in which code
                                                                               lengths are stored,
                                                                               as per the zlib
                                                                               specification */
    int i;
 ...
 * @return 0 for success, -1 for failure
 */
static int em_lsb_huffman_decoder_read_var_lengths (em_lsb_huffman_decoder_t* pTablesDecoder,
                                                    const unsigned int* pTablesRevSymbolTable,
                                                    const int nReadSymbols,
                                                    const int nSymbols,
                                                    unsigned char* pCodeLength,
                                                    em_lsb_bitreader_t* pBitReader)
{
    ...
    while (i < nReadSymbols)
    {
        unsigned int nLength =
            em_lsb_huffman_decoder_read_value (pTablesDecoder, pTablesRevSymbolTable, pBitReader);
        if (nLength == -1)
            return -1;
...
static size_t em_inflate_copy_stored (em_lsb_bitreader_t* pBitReader,
                                      unsigned char* pOutData,
                                      size_t nOutDataOffset,
                                      size_t nBlockMaxSize)
{
    ...
    if ((pBitReader->pInBlock + 4) > pBitReader->pInBlockEnd)
        return -1;

    /* Read block length, and its two's complement verification value */
    unsigned short nStoredLen = ((unsigned short) pBitReader->pInBlock[0]) |
                                (((unsigned short) pBitReader->pInBlock[1]) << 8);
    pBitReader->pInBlock += 2;

    unsigned short nNegStoredLen = ((unsigned short) pBitReader->pInBlock[0]) |
                                   (((unsigned short) pBitReader->pInBlock[1]) << 8);
    pBitReader->pInBlock += 2;

    /* Make sure that the len and the two's complement match */
    if (nStoredLen != ((~nNegStoredLen) & 0xffff))
        return -1;

    /* Make sure there is room */
    if (nStoredLen > nBlockMaxSize)
        return -1;
...
static const unsigned int em_inflate_matchlen_code[NMATCHLENSYMS] = {
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 0, 0),   MATCHLEN_PAIR (MIN_MATCH_SIZE + 1, 0),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 2, 0),   MATCHLEN_PAIR (MIN_MATCH_SIZE + 3, 0),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 4, 0),   MATCHLEN_PAIR (MIN_MATCH_SIZE + 5, 0),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 6, 0),   MATCHLEN_PAIR (MIN_MATCH_SIZE + 7, 0),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 8, 1),   MATCHLEN_PAIR (MIN_MATCH_SIZE + 10, 1),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 12, 1),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 14, 1),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 16, 2),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 20, 2),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 24, 2),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 28, 2),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 32, 3),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 40, 3),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 48, 3),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 56, 3),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 64, 4),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 80, 4),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 96, 4),  MATCHLEN_PAIR (MIN_MATCH_SIZE + 112, 4),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 128, 5), MATCHLEN_PAIR (MIN_MATCH_SIZE + 160, 5),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 192, 5), MATCHLEN_PAIR (MIN_MATCH_SIZE + 224, 5),
    MATCHLEN_PAIR (MIN_MATCH_SIZE + 255, 0),
};
 ...
 * @param nDynamicBlock non-zero for a dynamic huffman tables block, zero for a static huffman
 * tables block
 * @param nInBlockSize size of compressed data, in bytes
 * @param pOutData pointer to output decompression buffer (previously decompressed bytes + room for
 * decompressing this block)
 * @param nOutDataOffset starting index of where to store decompressed bytes in output buffer (and
 * size of previously decompressed bytes)
 * @param nBlockMaxSize total size of output decompression buffer, in bytes
 *
 * @return size of decompressed data in bytes, or -1 for error
 */
static size_t em_inflate_decompress_block (em_lsb_bitreader_t* pBitReader,
                                           int nDynamicBlock,
                                           unsigned char* pOutData,
                                           size_t nOutDataOffset,
                                           size_t nBlockMaxSize)
{
    ...
    if (nDynamicBlock)
    {
        ...
        /* Read the encoded number of literals/match len symbols, offset symbols, and code length
         * symbols */
        unsigned int nLiteralSyms = em_lsb_bitreader_get_bits (pBitReader, 5);
        if (nLiteralSyms == -1)
            return -1;
        nLiteralSyms += 257;
        if (nLiteralSyms > NLITERALSYMS)
            return -1;
        ...
        if (nCodeLenSyms == -1)
            return -1;
        nCodeLenSyms += 4;
        if (nCodeLenSyms > NCODELENSYMS)
            return -1;

        /* Read code lengths table */
        if (em_lsb_huffman_decoder_read_raw_lengths (NCODELENBITS /* code length bits */,
                                                     nCodeLenSyms /* symbols */,
                                                     NCODELENSYMS,
                                                     nCodeLength,
                                                     pBitReader) < 0)
            return -1;
        if (em_lsb_huffman_decoder_prepare_table (&tablesDecoder,
                                                  nTablesRevSymbolTable,
                                                  NCODELENSYMS,
                                                  NCODELENSYMS,
                                                  nCodeLength) < 0)
            return -1;
        ...
        if (em_lsb_huffman_decoder_read_var_lengths (&tablesDecoder,
                                                     nTablesRevSymbolTable,
                                                     nLiteralSyms + nOffsetSyms /* read symbols */,
                                                     NLITERALSYMS + NOFFSETSYMS /* total symbols */,
                                                     nCodeLength,
                                                     pBitReader) < 0)
            return -1;
        if (em_lsb_huffman_decoder_prepare_table (&literalsDecoder,
                                                  nLiteralsRevSymbolTable,
                                                  nLiteralSyms,
                                                  NLITERALSYMS,
                                                  nCodeLength) < 0)
            return -1;
        if (em_lsb_huffman_decoder_prepare_table (&offsetDecoder,
                                                  nOffsetRevSymbolTable,
                                                  nOffsetSyms,
                                                  NOFFSETSYMS,
                                                  nCodeLength + nLiteralSyms) < 0)
            return -1;
    }
    else
    {
        ...
        /* Fixed huffman codes for this block. Build fixed code lengths for literals/match lens and
         * offsets, as per the zlib specification */
        ...
        if (em_lsb_huffman_decoder_prepare_table (&literalsDecoder,
                                                  ...
                                                  NLITERALSYMS,
                                                  NLITERALSYMS,
                                                  nFixedLiteralCodeLen) < 0)
            return -1;
        if (em_lsb_huffman_decoder_prepare_table (&offsetDecoder,
                                                  nOffsetRevSymbolTable,
                                                  NOFFSETSYMS,
                                                  NOFFSETSYMS,
                                                  nFixedOffsetCodeLen) < 0)
            return -1;
    }

    /* Replace codewords in the offsets table by the base + extra bit count, save table lookups when
     * decoding */
    for (i = 0; i < NOFFSETSYMS; i++)
    {
    ...
    /* Replace match codewords in the literals/match len table by the base + extra bit count, save
     * table lookups when decoding */
    for (i = 0; i < NLITERALSYMS; i++)
    {
        ...
        if (n >= NMATCHLENSYMSTART && n < NLITERALSYMS)
        {
            ...
            {
                /* Handle 286 and 287 codes, used for constructing a complete canonical tree,
                 * present in static huffman tables but not used */
    ...
    while (1)
    {
        ...
        unsigned int nLiteralsCodeword = em_lsb_huffman_decoder_read_value (&literalsDecoder,
                                                                            nLiteralsRevSymbolTable,
                                                                            pBitReader);
        ...
        {
            if (nLiteralsCodeword == NEODMARKERSYM)
                break; /* EOD marker, all done */
            if (nLiteralsCodeword == -1)
                return -1;

            /* 256..284: match. decode match length. */

            unsigned int nMatchLen =
                em_lsb_bitreader_get_bits (pBitReader, (nLiteralsCodeword >> 16) & 15);
            if (nMatchLen == -1)
                return -1;
            ...
            unsigned int nOffsetCodeword = em_lsb_huffman_decoder_read_value (&offsetDecoder,
                                                                              nOffsetRevSymbolTable,
                                                                              pBitReader);
            if (nOffsetCodeword == -1)
                return -1;

            unsigned int nMatchOffset =
                em_lsb_bitreader_get_bits (pBitReader, (nOffsetCodeword >> 16) & 15);
            if (nMatchOffset == -1)
                return -1;
...
http://create.stephan-brumme.com and its sub-pages is licensed similar to the zlib license:

This software is provided 'as-is', without any express or implied warranty. In no event will the
author be held liable for any damages arising from the use of this software. Permission is granted
to anyone to use this software for any purpose, including commercial applications, and to alter it
and redistribute it freely, subject to the following restrictions: The origin of this software must
not be misrepresented; you must not claim that you wrote the original software. If you use this
software in a product, an acknowledgment in the product documentation would be appreciated but is
not required. Altered source versions must be plainly marked as such, and must not be misrepresented
...
static unsigned int em_inflate_crc32_4bytes (const void* data,
                                             size_t length,
...
typedef enum
{
    EM_INFLATE_CHECKSUM_NONE = 0,
    EM_INFLATE_CHECKSUM_GZIP,
    EM_INFLATE_CHECKSUM_ZLIB
} em_inflate_checksum_type_t;
 ...
 */
size_t em_inflate (const void* pCompressedData,
                   ...
                   size_t nMaxOutDataSize)
{
    ...
    if ((pCurCompressedData + 2) > pEndCompressedData)
        return -1;
    ...
    if (pCurCompressedData[0] == 0x1f && pCurCompressedData[1] == 0x8b)
    {
        ...
        if ((pCurCompressedData + 8) > pEndCompressedData ||
            pCurCompressedData[0] != 0x08 /* deflate */)
            return -1;
        pCurCompressedData++;
        ...
        if (flags & 0x04)
        { /* Extra field present, starts with two-byte length */
            if ((pCurCompressedData + 2) > pEndCompressedData)
                return -1;
            unsigned short nExtraFieldLen = ((unsigned short) pCurCompressedData[0]) |
                                            (((unsigned short) pCurCompressedData[1]) << 8);
            pCurCompressedData += 2;

            if ((pCurCompressedData + nExtraFieldLen) > pEndCompressedData)
                return -1;
            pCurCompressedData += nExtraFieldLen;
        }

        if (flags & 0x08)
        { /* Original filename present, zero terminated */
            do
            {
                if (pCurCompressedData >= pEndCompressedData)
                    return -1;
                pCurCompressedData++;
        ...
        if (flags & 0x10)
        { /* File comment present, zero terminated */
            do
            {
                if (pCurCompressedData >= pEndCompressedData)
                    return -1;
    ...
    else if ((pCurCompressedData[0] & 0x0f) == 0x08)
    {
        ...
        if ((CMF >> 4) <= 7 && (nCheck % 31) == 0)
        {
            ...
            pCurCompressedData += 2;
            if (FLG & 0x20)
            { /* Preset dictionary present */
                if ((pCurCompressedData + 4) > pEndCompressedData)
                    return -1;
    ...
    do
    {
        ...
        {
            case 0: /* Stored */
                nBlockResult = em_inflate_copy_stored (&bitReader,
                                                       pOutData,
                                                       nCurOutOffset,
                                                       nMaxOutDataSize - nCurOutOffset);
                break;

            case 1: /* Static huffman */
                nBlockResult = em_inflate_decompress_block (&bitReader,
                                                            0 /* static */,
                                                            pOutData,
                                                            nCurOutOffset,
                                                            nMaxOutDataSize - nCurOutOffset);
